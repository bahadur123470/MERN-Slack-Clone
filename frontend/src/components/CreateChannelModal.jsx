...
const CreateChannelModal = ({ onClose }) => {
    ...
    const [loadingUser, setLoadingUser] = useState(false); // loading state for user search
    const [_, setSearchParams] = useSearchParams(); // search params for user search

    const {client, setActiveChannel} = useChatContext();

    //  fetch user for member selection
    useEffect(() => {
    ...
    const validateChannelName = (name) => {
        ...
        if (name.length > 20) return "Channel name must be less than 20 characters.";
        return "";
    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            .replace(/\s+/g, "-").replace(/[^a-z0-9-_]/g, "").slice(0, 20)

            //  prepare the channel data
            const channelData = {
                name: channelName.trim(),
                created_by_id: client.user.id,
                member: [client.user.id, ...selectedMembers],
            };
                    ...
                    {channelType === "private" && (
                        <div className='form-group'>
                            ...
                            <div className='member-selection-header'>
                                <button 
                                    type='button' 
                                    className='btn bt-secondary btn-small'
